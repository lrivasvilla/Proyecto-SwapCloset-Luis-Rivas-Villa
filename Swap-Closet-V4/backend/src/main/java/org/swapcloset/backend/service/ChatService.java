...
public class ChatService {
    ...
    public ChatDTO save(ChatDTO dto) {
        if (dto == null) throw new IllegalArgumentException("ChatDTO no debe ser null");
        if (dto.getUsuario1Id() == null || dto.getUsuario2Id() == null || dto.getProducto1Id() == null)
            throw new IllegalArgumentException("usuario1Id, usuario2Id y producto1Id son obligatorios al crear un chat");
    ...
    public ChatDTO update(ChatDTO dto) {
        if (dto == null || dto.getId() == null)
            throw new IllegalArgumentException("ChatDTO y su id no deben ser null");

        Chat existente = chatRepository.findById(dto.getId())
                .orElseThrow(() -> new IllegalArgumentException("No existe Chat con id: " + dto.getId()));

        // Actualización parcial de fechas
        ...
        if (dto.getEstadoIntercambio() != null) {
            ...
            try {
                ...
                throw new IllegalArgumentException("Estado de intercambio inválido. Valores permitidos: pendiente, aceptado, devuelto");
            }
        }
